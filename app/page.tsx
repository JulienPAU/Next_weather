...
import type { WeatherData, Location } from "./utils/types/weather";
...
const WeatherPage = () => {
  ...
  const loadWeatherData = async (
    ...
    setLoading(true);
    try {
      const { weatherData, location: newLocation } = await fetchWeatherData(
        lat,
        long,
        cityName
      );
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              const cityName = await fetchCityNameFromCoordinates(
                latitude,
                longitude
              );
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center mt-10 flex flex-col justify-center items-center text-xl">
        ...
        <button
          type="button"
          className="bg-transparent hover:bg-blue-500 mt-10 text-white font-semibold hover:text-white py-2 px-4 border border-white hover:border-transparent rounded"
        >
          <a href="/">Réessayez</a>
        </button>
      </div>
      ...
      <div className="flex flex-wrap flex-col justify-evenly p-4  w-full">
        <div className="rounded-3xl  flex flex-wrap flex-col gap-3 items-center justify-evenly text-center sm:text-left sm:flex-1 shadow-light dark:shadow-dark my-5">
          ...
          <div className="flex flex-wrap w-full  lg:gap-1 sm:gap-3 justify-evenly">
              ...
              .map((forecast, index) => (
                <WeatherCard
                  ...
                  icon={getWeatherIcon(
                    ...
                    isDayTime(forecast.time, weatherData.timezone) ? 1 : 0,
            ...
            {showMore &&
              hourlyForecast
                .slice(currentHourIndex + 6, currentHourIndex + 11)
                .map((forecast, index) => (
                  <WeatherCard
                    key={index}
                    icon={getWeatherIcon(
          ...
          <button
            ...
            className="mt-2 mb-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-600 transition-colors"
          >
        ...
        <div className="rounded-3xl  flex flex-wrap flex-col gap-3 items-center justify-evenly text-center sm:text-left sm:flex-1 shadow-light dark:shadow-dark my-5">
          ...
          <div className="flex flex-wrap w-full  lg:gap-1 sm:gap-3 justify-evenly">
            {dailyForecast
              .slice(currentDayIndex - 1, currentDayIndex + 4)
              .map((forecast, index) => (
                <WeatherCard
                  key={index}
